...
if( $conn ) {
    ...
    $tsql = "SELECT ORDER_QTY,INVS_NUM,PROD_CD,LOG_DT,UNIT_PRS from  dbo.invt_log WHERE INVS_NUM < 99999999 ORDER BY INVS_NUM" ;  
            ...
            if($row['LOG_DT']>=$month18&&$row['LOG_DT']<=$month181){
              
              $prodx18=$prodx18+$row['UNIT_PRS']*$row['ORDER_QTY'];
              $count18=$count18+$row['UNIT_PRS']*$row['ORDER_QTY'];
              
              
            }else if($row['LOG_DT']>=$month17&&$row['LOG_DT']<=$month171){
              
              $prodx17=$prodx17+$row['UNIT_PRS']*$row['ORDER_QTY'];
              $count17=$count17+$row['UNIT_PRS']*$row['ORDER_QTY'];
              
            }else if($row['LOG_DT']>=$month16&&$row['LOG_DT']<=$month161){
              
              $prodx16=$prodx16+$row['UNIT_PRS']*$row['ORDER_QTY'];
              $count16=$count16+$row['UNIT_PRS']*$row['ORDER_QTY'];
              
            }else if($row['LOG_DT']>=$month15&&$row['LOG_DT']<=$month151){
              
              $prodx15=$prodx15+$row['UNIT_PRS']*$row['ORDER_QTY'];
              $count15=$count15+$row['UNIT_PRS']*$row['ORDER_QTY'];
             ...
             if($lastdate>=$month18&&$lastdate<=$month181){
              
             $sum18a++;  }
             else if($lastdate>=$month17&&$lastdate<=$month171){
             
             $sum17a++;  }
             else if($lastdate>=$month16&&$lastdate<=$month161){
              
             $sum16a++;  }
             else if($lastdate>=$month15&&$lastdate<=$month151){
...
<script>
var data = [
  {letter: "2015",    frequency:  <?php echo $sum15; ?>,},
  {letter: "2015 total",    frequency:  <?php echo $sum15a; ?>,},
  {letter: "2016",    frequency:  <?php echo $sum16; ?>,},
  {letter: "2016 total",    frequency:  <?php echo $sum16a; ?>,},
  {letter: "2017",    frequency:  <?php echo $sum17; ?>,},
  {letter: "2017 total",    frequency:  <?php echo $sum17a; ?>,},
  {letter: "2018",     frequency: <?php echo $sum18; ?>,},
  {letter: "2018 total",     frequency: <?php echo $sum18a; ?>,},
